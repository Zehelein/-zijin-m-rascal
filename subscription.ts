...
import { BrokerAsPromised, SubscriptionSession, AckOrNack } from "rascal";
...
export default class SubScription {
  ...
  constructor(consumer: Consumer, broker: BrokerAsPromised) {
    this.consumer = consumer;
    this.broker = broker;
  }

  public async subscribe() {
    this.session = await this.broker.subscribe(
      this.consumer.name,
      this.consumer.overrides
    );
  ...
  private getRecovery() {
    const consumeRecovery =
      this.broker.config.subscriptions?.[this.consumer.name]?.recovery || "";
